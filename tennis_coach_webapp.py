...
def get_session_messages(player_record_id: str, session_id: str) -> list:
    ...
    try:
        # DEBUG LINES:
        st.error(f"DEBUG: Looking for messages with player_id={player_record_id}, session_id={session_id}")
        
        url = f"https://api.airtable.com/v0/appTCnWCPKMYPUXK0/Active_Sessions"
        headers = {"Authorization": f"Bearer {st.secrets['AIRTABLE_API_KEY']}"}
        
        session_id_number = int(''.join(filter(str.isdigit, session_id))) if session_id else 1
        st.error(f"DEBUG: Converted session_id to number: {session_id_number}")
        ...
        st.error(f"DEBUG: Raw response: {response.text[:500]}")
        
        if response.status_code == 200:
            records = response.json().get('records', [])
            st.error(f"DEBUG: Found {len(records)} records in Active_Sessions")
            messages = []
            for record in records:
        ...
        st.error(f"DEBUG: Exception in get_session_messages: {str(e)}")
        return []
